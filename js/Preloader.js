Preloader = function(rootFolder, scene, game, options) {

    var txt = options.textId ||'loadingValue';
    /**
     * ID of the html element to update during progress
     * @type {string}
     */
    this.loadingText = document.getElementById(txt);
    /**
...
Preloader.prototype = {
    ...
    update : function() {
        this.progress = Math.round((this.fileLoadedSuccess + this.fileLoadedError) / (this.fileToLoad.length) * 100);
        this.loadingText.innerHTML = this.progress + "%";
    },
    /**
